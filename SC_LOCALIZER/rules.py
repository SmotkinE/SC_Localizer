...
# Версия набора умолчаний. Поднимается, когда какой-то категории меняют
# значение по умолчанию.
#
# Зачем: сохранённый профиль всегда перебивает умолчания — иначе выбор человека
# сбрасывался бы при каждом обновлении. Но из-за этого правка enabled_by_default
# не доезжала ни до кого, у кого программа уже запускалась: значение в его
# profile.json осталось со старой сборки, хотя он его сознательно не выбирал.
#
# Поэтому переключение умолчания оформляется записью здесь: при обновлении
# профиля перечисленным категориям один раз навязывается новое значение,
# а всё остальное, что человек настроил, остаётся как было.
DEFAULTS_VERSION = 2

_DEFAULT_CHANGES: dict[int, tuple[str, ...]] = {
    # 1.3.1: режим называется «Только описания», а датапады и подсказки —
    # это описания. Включены по умолчанию.
    2: ('lore_text', 'hints'),
}


def defaults_changed_since(version: int) -> set[str]:
    """Категории, у которых умолчание менялось после указанной версии."""
    changed: set[str] = set()
    for v, ids in _DEFAULT_CHANGES.items():
        if v > version:
            changed.update(ids)
    return changed
